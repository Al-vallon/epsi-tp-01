...
export class BookListComponent implements OnInit {
  ...
  loadBooks(): void {
    this.bookService.getBooks().subscribe({
      next: (books: Book[]) => {
        this.books = books;
        // console.log(this.books , "list books");
      },
      error: (err: any) => {
  ...
  toggleFavorite(book: Book): void {
    this.bookService.toggleFavorite(book.id).subscribe({
      next: (updatedBook: Book) => {
        ...
        window.alert(`Le favori de "${updatedBook.title}" a été modifié avec succès !`);
      },
      error: (err: any) => {
        // TODO 17: Affiche une alerte qui indique que la modification du favori a échoué
        window.alert('Erreur lors de la modification du favori. Veuillez réessayer.');
  ...
  deleteBook(id: string): void {
    this.bookService.deleteBook(id).subscribe({
      ...
      error: (err: any) => {
        ...
        window.alert('Erreur lors de la suppression du livre. Veuillez réessayer.');
